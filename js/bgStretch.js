...
var discription;
var discriptionBtn;
var discriptionNum = -1;
var discriptionNumPrev = -1;
var discriptionOpen = false;

$(document).ready(function(){
    discription = $('#description .info li');
    discriptionBtn = $('#description .info a')
    discriptionBtn.click(function(){
        if(discriptionNum != -1){
            if(!discriptionOpen){
                discriptionOpen = true;
                discription.eq(discriptionNum).css({display:'block'}).stop().animate({left:'0px'}, 550, 'easeOutBack');
            } else {
                discriptionOpen = false;
                discription.eq(discriptionNum).stop().animate({left:'-700px'}, 500, 'easeInCubic', function(){
			...
			var th=$(this),
				...
				_={
					...
					rfrshFu:function(){
						...
						_.li.each(function(n){
                            ...
                            if(discriptionNum == -1){
                                discriptionNum = _.n;
                                discriptionOpen = false;
                                 discription.eq(discriptionNumPrev).stop().animate({left:'-700px'}, 300, 'easeInCubic', function(){
                                    $(this).css({display:'none'})
                                })
                            }
                            ...
                            if(!th.hasClass(_.actCl)){
                                ...
                                $('.description li').eq(discriptionNum).css({display:'block'}).stop().animate({right:'32px'}, 330);
                                $('.description li').eq(discriptionNumPrev).stop().animate({bottom:-$('.description li').eq(discriptionNumPrev).height()-70}, 330, function(){
                                    $(this).css({bottom:'0px', right:'-450px', display:'none'});
                                });
                                
                                $('#description .counter .current').text(discriptionNum + 1);
                            }
						})						
					},
					markFu:function(){
					...
					chngFu:function(n){
                        ...
                        discriptionNumPrev = discriptionNum
                        discriptionNum = -1;
